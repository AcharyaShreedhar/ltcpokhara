...
async function addNotices(req, res) {
  const addNoticesQuery = `INSERT INTO notices (notice_title, notice_cat, notice_approvedby, notice_publisheddate, notice_content,notice_file) values (?,?,?,?,?,?)`;
  pool.query(
    addNoticesQuery,
    [
...
async function updateNotices(req, res) {
  const updateNoticesQuery = `UPDATE notices SET notice_title=?, notice_cat=?, notice_approvedby=?, notice_publisheddate=?, notice_content=?, notice_file=? WHERE notice_id=?`;
  pool.query(
    updateNoticesQuery,
    [
      req.body.notice_title,
